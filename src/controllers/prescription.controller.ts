...
export const getHealthTimelineController = async (req: Request, res: Response) => {
  if (!req.userId) throw createServiceError("User not authenticated", 401);
  const result = await getHealthTimelineService(req.userId);
  res.status(200).json(createSuccessResponse(result));
};

export const getCaseDocumentationController = async (req: Request, res: Response) => {
  if (!req.userId) throw createServiceError("User not authenticated", 401);
  const result = await getCaseDocumentationService(req.userId);
  res.status(200).json(createSuccessResponse(result));
};