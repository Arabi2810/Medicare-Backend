...
`;
  ...
  prescriptions.forEach((p) => {
    p.medicines.forEach((m) => allMedicines.push(m.name));
    p.diagnosis.forEach((d) => allDiagnoses.push(d));
  });

  if (allMedicines.length === 0) {
    return "No active medicines found to analyze.";
  }

  const prompt = `You are a clinical pharmacologist. A patient is currently taking multiple medicines from different prescriptions at the same time.

Patient's current active medicines: ${[...new Set(allMedicines)].join(", ")}
Patient's current diagnoses: ${[...new Set(allDiagnoses)].join(", ")}

Perform a Side Effect and Drug Interaction Analysis:

1. **Possible Side Effects**: For each medicine, list the most common side effects in simple language a patient can understand.
2. **Drug Interactions**: Identify any dangerous or concerning interactions between these medicines taken together.
3. **Warning Signs**: Tell the patient what symptoms to watch for that would require immediate medical attention.
4. **Safe Usage Tips**: Practical advice on how to take these medicines safely (timing, food, etc).
...
export const getHealthTimelineService = async (userId: string): Promise<string> => {
  ...
  if (prescriptions.length === 0) {
    return "No prescription history found to generate timeline.";
  }

  const prescriptionSummaries = prescriptions.map((p, index) => ({
    visit: index + 1,
    date: p.uploadedAt,
    ...
    tests: p.tests.map((t) => t.name),
    isComplete: p.isComplete,
  }));

  const prompt = `You are a medical historian. Based on this patient's prescription history, generate a Longitudinal Health Timeline.

Prescription History:
${JSON.stringify(prescriptionSummaries, null, 2)}

Generate a chronological health timeline that:
1. **Health Journey**: Describe the patient's health journey from first to latest visit in simple narrative form.
2. **Disease Progression**: Identify any recurring conditions, chronic diseases, or improving conditions over time.
3. **Medicine Patterns**: Note any medicines that appear repeatedly and what this suggests.
...
export const getCaseDocumentationService = async (userId: string): Promise<string> => {
  ...
  if (prescriptions.length === 0) {
    return "No prescription data found to generate case documentation.";
  }
  ...
  prescriptions.forEach((p) => {
    p.medicines.forEach((m) => allMedicines.add(m.name));
    p.diagnosis.forEach((d) => allDiagnoses.add(d));
    p.symptoms.forEach((s) => allSymptoms.add(s));
    p.tests.forEach((t) => allTests.add(t.name));
  });
